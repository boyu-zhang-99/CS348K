...
class SMPLDeformer():
    def __init__(self, model_path, gender, threshold=0.07, k=1) -> None:
    ...
    def deform(self, pts):
        ...
        with torch.no_grad():
            #indices = torch.randperm(self.vertices.shape[1])[:self.vertices.shape[1] // 2]  # Randomly select 1/3 of the indices
            #sampled_vertices = self.vertices[:,indices]  # Use the random indices to sample from the vertices
            #dist_sq, idx, _= ops.knn_points(pts.float(), sampled_vertices.float(), K=self.k)
