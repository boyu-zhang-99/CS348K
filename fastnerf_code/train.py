...
def main(opt):
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        ...
        monitor="train/psnr",
        filename="epoch={epoch:04d}-train_psnr={train/psnr:.1f}",
        mode="max",
    ...
    early_stop_callback = pl.callbacks.EarlyStopping(monitor="train/psnr", 
                                                     patience=25, 
                                                     min_delta=0.00,
                                                     mode="max",
                                                     check_on_train_epoch_end=True)
    
    pl_logger = CSVLogger(save_dir="log", name="train", version="0")
